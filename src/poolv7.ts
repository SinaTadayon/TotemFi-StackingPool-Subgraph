...
import { StakingPoolV8 as StakingPoolContract } from "../generated/templates"
import { PoolCreated } from "../generated/StakingPoolFactoryV8/StakingPoolFactoryV8"
import {
    Stake as StakeEvent,
...
} from "../generated/templates/StakingPoolV8/StakingPoolV8"
import * as summary from "./summary"
import { BatchStake } from "../generated/templates/StakingPoolV9/StakingPoolV9"
...
export function loadOrCreateWithdraw(address: Address): Withdraw {
    let withdraw = Withdraw.load(address.toHex())

    if (withdraw == null) {
        withdraw = new Withdraw(address.toHex())
        withdraw.stakingReward = BigInt.fromI32(0)
        withdraw.tokenReward = BigInt.fromI32(0)
        withdraw.totmPrize = BigInt.fromI32(0)
        withdraw.user = ""
    }
...
export function handleStake(event: StakeEvent): void {
    ...
    entity.idx = event.params.indexOfStake
    entity.save()
...
export function handleWithdrawWrappedTokenPrize(event: WithdrawEventToken): void {
    // save entity
    let entity = new Withdraw(event.transaction.hash.toHex()+"_token_prize")
    entity.pool = event.address.toHex()
    entity.user = event.params.user.toHex()
...
export function handleWithdrawTotemPrize(event: WithdrawEventTotmPrize): void {
    // save entity
    let entity = new Withdraw(event.transaction.hash.toHex()+"_totem_prize")
    entity.pool = event.address.toHex()
    entity.user = event.params.user.toHex()
...
export function handleWithdrawStakingReturn(event: WithdrawEventReturn): void {
    // save entity
    let entity = new Withdraw(event.transaction.hash.toHex()+"_staking_return")
    entity.pool = event.address.toHex()
    entity.user = event.params.user.toHex()
...
export function handleBatchStake(event:BatchStake) : void {
    const preds = event.params.predictions
    var id= ""
    var totalStaked : BigInt = BigInt.fromI32(0)
    for (let i = 0 ; i < preds.length;i++){
        if (i == 0 ) {
            id = event.transaction.hash.toHex()
        }else{
            id = event.transaction.hash.toHex()+"_"+i.toString()
        }
        // save entity
        let entity = new Stake(id)
        entity.pool = event.address.toHex()
        entity.user = event.params.user.toHex()
        entity.value = event.params.stakeAmounts[i]
        entity.pricePrediction = event.params.predictions[i]
        entity.timestamp = event.block.timestamp
        // entity.idx = event.params.indexOfFirstStake.plus(BigInt.fromI32(i))
        entity.save()

        totalStaked =  totalStaked.plus(event.params.stakeAmounts[i])
    }
    // save user
    let user = loadOrCreateUser(event.params.user)
    user.totalStaked = user.totalStaked.plus(totalStaked)
    user.totalStakes = user.totalStakes.plus(BigInt.fromI32(preds.length))
    user.timestamp = event.block.timestamp
    user.save()

    let pool = loadOrCreatePool(event.address)
    pool.totalStakes = pool.totalStakes + preds.length
    pool.totalStaked = pool.totalStaked.plus(totalStaked)
    pool.timestamp =event.block.timestamp
    pool.save()
